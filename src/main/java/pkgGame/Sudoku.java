...
public class Sudoku extends LatinSquare {
	...
	private void SetCells() {
		for (int Row = 0; Row < iSize; Row++) {
			for (int Col = 0; Col < iSize; Col++) {
				...
				c.setlstValidValues(getAllValidCellValues​(Col, Row));
	...
	private HashSet<Integer> getAllValidCellValues​(int Col, int Row) {
		HashSet<Integer> hsPotValidValues = new HashSet<Integer>();
		
		for (int potval = 1; potval <= iSize; potval++) {
			if (isValidValue(Row, Col, potval)) {
				hsPotValidValues.add(potval);
				}
			}
		return hsPotValidValues;
		}
	
	public boolean isValidValue​(Cell c, int val) {
		return this.isValidValue(c.getiRow(), c.getiCol(), val);
		}
	...
	private boolean fillRemaining​(Cell c) {
		...
		for (int potvalidval: c.getlstValidValues()) {
			if (isValidValue​(c, potvalidval)) {
				this.getPuzzle()[c.getiRow()][c.getiCol()] = potvalidval;
	...
	private class Cell{
		...
		public void setlstValidValues(HashSet<Integer> LstValidValues) {
			lstValidValues = new ArrayList<Integer>(LstValidValues);
		}
		...
		public Cell GetNextCell​(Cell c) {
			...
			if (c.getiRow() >= iSize - 1 && c.getiCol() + 1 >= iSize) {
				return null;
			}
